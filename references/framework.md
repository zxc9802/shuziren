...
1. `task.json`: exact script, duration, aliases, `operating_mode`, pending or auto-locked image/preview/voice/material choices, recorded `view_mode`, settings, overrides. `scripts/plan_job.py --auto` writes the locked auto defaults instead of `pending`.
...
- `approve-raw ...`
- `approve-raw-auto --recorded-at ... --evidence-ref ...`
