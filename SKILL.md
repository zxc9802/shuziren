...
2. Record MiniMax `huangxu1`, no company materials, `original_image1`,
   `front`, and preview disabled. Do not ask MiniMax vs HeyGen, company
...
5. Resolve the existing Avatar IV look for `original_image1`. Write
   `voice-plan.json`, synthesize MiniMax, upload the exact audio, and submit
   one structured full raw: portrait `9:16`, `720P`, captions off.
6. Persist and poll the same video ID. Run full-file QA including the
   realism gate. QA failure is a hard stop.
...
Read `references/performance-profiles.md` and `references/realism.md` before building the motion block.
...
repeat MiniMax synthesis, image generation, avatar creation, or paid video
